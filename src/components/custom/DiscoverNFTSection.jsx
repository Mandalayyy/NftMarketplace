...
import { StrapiImage } from "@/components/custom/StrapiImage";
  ...
  export async function DiscoverNFTSection({ data }) {
    const { title,description, link, nfts} = data;

    
    return (
      <div className="grid gap-40 py-40 md:grid-cols-2 md:grid-flow-row  items-end">
        ...
        <div className="order-3 flex gap-12 justify-center py-20 w-full items-center md:w-fit md:px-50  rounded-img border-2  border-action  lg:col-span-1 lg:justify-self-end  transition-all ease-in duration-300 hover:scale-95" >
          <div>
        ...
        <div className="md:order-3 grid gap-20  lg:grid-cols-3 lg:gap-30 md:grid-flow-row md:grid-cols-2 md:col-span-2 lg:col-span-3">
          {nfts.data.map((nft, index)=> (
            <div className={`bg-bgSecondary  rounded-img md:first:hidden lg:first:grid transition-all ease-in duration-300 hover:scale-95`} key={nft.id}>
            <Link href={`/nft/${nft.id}`}  className="">
              <StrapiImage className="rounded-t-img w-full" src={nft.nftImg.url} width={1200} height={800} alt='avatar'/>
                  <div className="flex gap-25 flex-col px-20 pt-20 pb-25">
                    <div className="">
                      <h5 className="font-semibold text-md leading-relaxed">{nft.nftName}</h5>
                      <Link href={`/user/${nft.nftOwner.id}`} className="flex gap-12">
                        <StrapiImage src={nft.nftOwner.avatar.url} className="w-24 h-24" alt='avatar' height="48" width='48' />
                        <p className="font-secondary text-base font-normal leading-relaxed">{nft.nftOwner.username}</p>
                      </Link>
                    </div>
                    <div>
                      <div>
