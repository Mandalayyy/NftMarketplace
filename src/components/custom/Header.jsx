...
export async function Header({ data }) {
    const { signUp, navlinks } = data;
    const user = await getUserMeLoader()
    
    return (
        <div className="flex justify-between px-30 py-15 items-center">
            <Logo className="flex gap-2" />
            <div className="hidden lg:flex items-center gap-30">
                <div className="flex gap-30">

                    {navlinks.map((link) => {
                        return (
                            <Link className="text-base  leading-relaxed font-semibold" key={link.id} href={link.url}>
                                {link.text}
                            </Link>
                        );
                    })}
                    ...
                    {user.ok ?
                     ( <Link className="text-base  leading-relaxed font-semibold" href={`/user/${user.data.id}`}>My Profile</Link>)
                     : 
                     ('')}
                </div>
                <div className="bg-action items-center px-30 py-20 flex gap-12 rounded-img">
                    <span>
                        <svg className="w-20 h-20" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="flex items-center">
                        ...
                        : 
                        (<div className="flex">
                            <Link className="text-base leading-6" key={signUp.id} href={signUp.url}>
                                {signUp.text}
                            </Link>
                        </div>)}
            ...
            <div className="lg:hidden">
                <BurgerMenu navlinks={navlinks} signUp={signUp} />
            </div>
        </div>
    );
}
